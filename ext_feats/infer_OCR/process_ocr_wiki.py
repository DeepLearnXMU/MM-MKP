...
def tw_tokenize(src_fn, trg_fn, wiki_fn,ocr_dict, valid_token_set):
    ...
    for idx, (line,line2) in enumerate(zip(fr_list,fr2_list)):
        if 1:
            ...
            #wiki_list=[item.split('##') for item in wiki_list]
            re_wiki_list=[]
            for item in wiki_list:
                tmp=item.split('##')
                re_tmp=tmp[1]+': '+tmp[2]
                re_wiki_list.append(re_tmp)
            re_wiki='; '.join(re_wiki_list)
            
            #print("re_wiki",re_wiki)

            wiki_tokens = process_sentence(re_wiki)
            #print("re_wiki1",' '.join(wiki_tokens))
            #wiki_tokens = list(filter(lambda x: re.match('^[a-zA-Z]+$', x), wiki_tokens))
            wiki_tokens = list(filter(lambda x: re.match('^[a-zA-Z0-9’!"#$%&\'()*+,-./:;<=>?@，。?★、…【】《》？“”‘’！[\\]^_`{|}~\s]+', x), wiki_tokens))
            #print("re_wiki2",' '.join(wiki_tokens))

            re_wiki=' '.join(wiki_tokens)
            if img_fn not in ocr_dict:
                ocr=''
                res=text+'<seg>'+ocr+'<seg>'+re_wiki+'<sep>'+kw+'<sep>'+jpg_url+'<sep>'+itm_pro+'<sep>'+itm+'<sep>'+wiki
                fw.write(res+'\n')
                continue
...
if __name__ == '__main__':
    ...
    src_fn = '../../data/tw_mm_s1/{}_src.txt'
    trg_fn = '../../data/tw_mm_s2/{}_ocr_final2.txt'
    wiki_fn='../../data/tw_mm_s1_ocr/{}_src_wiki.txt'
