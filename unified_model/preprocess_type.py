...
def read_src_trg_files(opt, tag="train"):
    ...
    for line in src_lines:
        ...
        src_array=np.array(src_words)
        tmp=np.argwhere(src_array=='<seg>')
        src_type=np.zeros(len(src_words))
        src_type[tmp[0][0]:]+=1
        src_type[tmp[1][0]:]+=1
        
        assert src_type.size==len(src_words)
    ...
    tokenized_pairs = list(zip(tokenized_src, tokenized_trg, img_fns,src_types))
...
def build_vocab(src_trg_pairs, vocab_size=40000):
    ...
    for src_word_list, trg_word_lists, _,_ in src_trg_pairs:
...
def build_dataset(src_trg_pairs, vocab, trg_class_vocab, is_test):
    examples = []
    for idx, (source, target, img,src_type) in enumerate(src_trg_pairs):
        ...
        if is_test:
            examples.append({'src': src, 'trg': trgs[0], 'trg_class': trgs_class[0], 'img': img, 'type':src_type,'src_oov': src_oov,
                             'oov_list': oov_list,
                             'trg_copy': trg_copies[0], 'src_str': source, 'trg_str': target})
        else:
            examples.extend(
                [{'src': src, 'trg': trg, 'trg_class': trg_class, 'img': img, 'type':src_type,'src_oov': src_oov, 'oov_list': oov_list,
...
def main(opt):
    ...
    torch.save(train_data, open(opt.res_data_dir + '/train{}.pt'.format(save_tag), 'wb'))
    ...
    torch.save(valid_data, open(opt.res_data_dir + '/valid{}.pt'.format(save_tag), 'wb'))
    ...
    torch.save(test_data, open(opt.res_data_dir + '/test{}.pt'.format(save_tag), 'wb'))
