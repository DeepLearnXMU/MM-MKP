...
import pickle
import numpy as np
from modules import masked_mean, masked_max, Attention, CoAttention, MaskedSoftmax, MyMultiHeadAttention, MyMultiHeadAttention4Filtration
...
class MultimodalEncoder(nn.Module):
    def __init__(self, opt):
        ...
        self.use_gate=opt.itm
        #self.use_gate=False
        self.use_type=opt.use_type
        #opt.itm
        self.tag=opt.tag
        ...
        self.itm_classifer_final = nn.Linear(opt.bi_hidden_size, 2)
        ...
        self.fc_img = nn.Linear(opt.bi_hidden_size * 2, 1)
        ...
        if 'multi_head_att' in self.cur_model:
            ...
            self.img_self_attn=nn.ModuleList(
                    [MyMultiHeadAttention(n_head, self.raw_img_feat_size, d_kv, dropout=opt.dropout, need_mask=False,
            ...
            self.img_text_multi_head_att = nn.ModuleList(
                    [MyMultiHeadAttention(n_head, self.bi_hidden_size, d_kv, dropout=opt.dropout, need_mask=True,
                                          is_regu=self.is_regu)
                     for _ in range(stack_num)])
            self.img_text_multi_head_att2 = nn.ModuleList(
                    [MyMultiHeadAttention(n_head, self.bi_hidden_size, d_kv, dropout=opt.dropout, need_mask=True,
                                          is_regu=self.is_regu)
                     for _ in range(stack_num)])
            ...
            self.text_img_multi_head_att = nn.ModuleList(
                    [MyMultiHeadAttention(n_head, self.bi_hidden_size, d_kv, dropout=opt.dropout, need_mask=False)
                     for _ in range(stack_num)])
            '''self.fine_grained_filtration = nn.ModuleList(
                    [MyMultiHeadAttention4Filtration(self.bi_hidden_size, d_kv, dropout=opt.dropout, need_mask=False)
                     for _ in range(stack_num)])'''
            self.fine_grained_filtration = MyMultiHeadAttention4Filtration(self.bi_hidden_size, d_kv, dropout=opt.dropout, need_mask=False)
                           

            if 'attr_text_multi_head_att' in self.cur_model:
    ...
    def init_weights(self, emb_type, pad_idx):
        ...
        if emb_type == 'random':
            ...
            self.type_embedding.weight.data = torch.Tensor(weights)
            # self.embedding.weight.requires_grad = False
            print('Load glove embedding!')

        self.embedding.weight.data[pad_idx] = 0
        self.type_embedding.weight.data[pad_idx] = 0

    def get_text_memory_bank(self, src,type, src_lens, return_last_state=True,enforce_sorted=True):
        # embed the src post with embedding and Bi-GRU layers
        batch_size, max_src_len = list(src.size())
        if self.use_type and type is not None:
            src_embed = self.embedding(src)+self.type_embedding(type)  # [batch, src_len, emb_size]
        else:
            src_embed = self.embedding(src)
        #todo


        packed_input_src = nn.utils.rnn.pack_padded_sequence(src_embed, src_lens, batch_first=True, enforce_sorted=enforce_sorted)
    ...
    def get_img_memory_bank(self, img_feats):
        ...
        #print("img_feats1",img_feats.shape)
        if self.img_ext_model == 'vgg':
            #residual = img_feats
            #for enc_layer in self.img_self_attn:
            #    output,_ = enc_layer(img_feats, img_feats, img_feats)
            #img_feats=output+residual
            img_feats = img_feats.reshape(-1, img_feats.shape[2])
            
            #print("img_feats2",img_feats.shape)
            ...
            #residual = img_feats
            #for enc_layer in self.img_self_attn:
            #    img_feats,_ = enc_layer(img_feats, img_feats, img_feats)
            img_feats = img_feats.reshape(-1, img_feats.shape[2])

            img_feats = self.linear_img(img_feats)
            #img_feats=img_feats+residual
            img_feats = img_feats.view(batch_size, -1, img_feats.shape[-1])  # [batch_size, 49, bi_hidden_size]
    ...
    def forward(self, src, src_lens, src_mask, img, attr, type=None,return_last_state=True,only_itm=False,only_trg_en=False,enhanced_img = None):
        # get text features
        batch_size, seq = src.size()

        if only_trg_en:
            # src,src_lens,src_mask=self._pad(src)
            # src=src.to(self.device)
            # src_mask=src_mask.to(self.device)
            memory_bank, encoder_final_state = self.get_text_memory_bank(src, type,src_lens, return_last_state,enforce_sorted=False)
            if 'text_img_multi_head_att' in self.cur_model:
                ...
                enc_output = enhanced_img
            
                #for enc_layer in self.fine_grained_filtration:
                #    enc_output, attn = enc_layer(text_feat, enc_output,enc_output)
                enc_output, attn = self.fine_grained_filtration(text_feat, enc_output,enc_output)
                # print("target2img_attn",attn.size())
                return attn
            elif 'img_text_multi_head_att' in self.cur_model:
                #img_feat = self.get_img_feat(enhanced_img)
                enc_output =enhanced_img #[bsz,hidden]
                enhanced_img_feat = enc_output
                #print("img_text_enc_output",enc_output.size())

                #for enc_layer in self.fine_grained_filtration:
                #    enc_output, attn = enc_layer(enc_output, memory_bank, memory_bank, src_mask)
                enc_output, attn = self.fine_grained_filtration(enc_output, memory_bank, memory_bank, src_mask)
        ...
        memory_bank, encoder_final_state = self.get_text_memory_bank(src, type,src_lens, return_last_state)
        ...
        if 'multi_head_att' in self.cur_model:
            ...
            combined_feat=enc_output
            relation_pred= self.itm_classifer_final(combined_feat)

            
            if 'img_text_multi_head_att' in self.cur_model:
                if self.use_gate:
                    gate=torch.softmax(relation_pred,dim=-1)[:,1].unsqueeze(1).expand(batch_size,self.bi_hidden_size)
                else:
                    gate=torch.ones(batch_size,self.bi_hidden_size)
                    gate=gate.to(self.device)
                enc_output =img_feat*gate #[bsz,hidden]
                enhanced_img_feat = enc_output
                #print("img_text_enc_output",enc_output.size())
                ...
                if self.use_gate:
                    gate1=torch.softmax(relation_pred,dim=-1)[:,1].unsqueeze(1).expand(batch_size,49).unsqueeze(1).expand(batch_size,1,49)
                else:
                    gate1=torch.ones(batch_size,1,49)
                    gate1=gate1.to(self.device)

                enc_output=img_feats
                enhanced_img_feat = img_feats
                
                enc_output, attn = self.fine_grained_filtration(text_feat, enc_output,enc_output,gate1)
                enc_output = self.get_img_feat(enc_output)
                ...
                if only_itm:
                    return relation_pred

        return memory_bank, encoder_final_state, img_feats, attr_feats, combined_feat,attn, enhanced_img_feat
...
class MultimodalMixture(nn.Module):
    def __init__(self, opt):
        ...
        self.ranking_loss=nn.KLDivLoss(reduction='sum')
    ...
    def forward(self, src, src_lens, src_mask, src_oov, trg, trg_len, trg_mask, max_num_oov, img=None, attr=None,type=None, only_classifier=False,only_itm=False):
    # def forward(self, src, src_lens, src_mask, src_oov, trg, max_num_oov, img=None, attr=None,type=None, only_classifier=False,only_itm=False):
 
        # ============================= Encoding text, image, attribtues =========================

        if only_itm:
            relation_labels=self.encoder(src, src_lens, src_mask, img, attr,type,True,only_itm)
            return relation_labels

        memory_bank, encoder_final_state, img_feats, attr_feats, combined_feat,text2img_attn,enhanced_img_feat = \
        self.encoder(src, src_lens, src_mask, img, attr,type)
        ...
        if self.use_kl:
            ...
            trg2img_attn=self.encoder(trg, trg_len, trg_mask,img,attr,None,True,False,True,enhanced_img_feat)
            # trg2img_attn=self.encoder(trg, None, None,img,attr,None,True,False,True,enhanced_img_feat)
            # print(f'1:{trg2img_attn.size()},{text2img_attn.size()}')
            return decoder_dist_all, h_t_next, attention_dist_all, encoder_final_state, classifier_outputs,trg2img_attn, text2img_attn
