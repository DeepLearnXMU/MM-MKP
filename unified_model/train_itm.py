...
import run_batches
from my_io_itm import get_tweet_img_loader,get_itm_img_loader, Vocabulary
...
def prepare_data_loader(split_tag, vocab, trg_class_vocab, opt, is_shuffle=True):
    ...
    data_path = os.path.join(opt.data_path, '{}{}.pt'.format(split_tag,opt.data_suffix))
    ...
    data_loader = get_tweet_img_loader(data, vocab, trg_class_vocab,
                                       opt.use_text, opt.use_img, opt.use_attr,opt.use_type,  opt.use_bert_src,
...
def prepare_itm_data_loader(split_tag, vocab, trg_class_vocab,opt, is_shuffle=True):
    assert split_tag in ['train_itm', 'test_itm']
    ...
    data_loader = get_itm_img_loader(data, vocab, trg_class_vocab,
                                       opt.use_text, opt.use_img, opt.use_attr,opt.use_type, opt.use_bert_src,
                                       img_feats_fn, attr_feats_fn, url_map_fn,
                                       bert_feats_fn, src_str_map_fn,
                                       is_test=opt.is_test, only_classifier=opt.only_classifier, debug=opt.debug,
...
def main(opt):
    ...
    #todo
    if opt.itm:
        itm_train_loader = prepare_itm_data_loader('train_itm', vocab,trg_class_vocab,opt)
        itm_test_loader = prepare_itm_data_loader('test_itm', vocab, trg_class_vocab,opt)
        print('Finish preparing data load for itm train (%d batches) and valid (%d batches) with batch size: %d\n' %
              (len(itm_train_loader), len(itm_test_loader), opt.batch_size))
    ...
    if opt.fix_classifier:
        ...
        for param in model.linear_classifer_final.parameters():
            param.requires_grad = False
        for param in model.itm_classifer_final.parameters():
            param.requires_grad = False
        print('\n\n=============================Fix the classifier===========================')
    ...
    if not opt.itm:
        itm_train_loader=None
        itm_test_loader=None

    run_batches.train_valid_itm_mixture(model, optimizer, train_data_loader, valid_data_loader,itm_train_loader,itm_test_loader,start_epoch, opt)
    #else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-test_src', type=str, default='../data/{}/test_src.txt')
    ...
    parser.add_argument('-epochs_to_save', type=int, default=1, help='Empirically set 3 for seq2seq, 5 for classifier')
    ...
    parser.add_argument('-use_type', action="store_true", help='')
    ...
    opt.model_dir = '/home/sata/dyfff/CMKP/unified_model/models/{}-{}'.format(opt.cur_model, opt.model_tag)
    opt.model_save_dir = '/home/sata/dyfff/CMKP/unified_model/models/{}-{}-{}/'.format(opt.cur_model, opt.data_tag, opt.seed)
