    ...
    with torch.no_grad():
        ...
        for batch_i, batch in enumerate(data_loader):
            if opt.use_type:
                ...
                src_str_list, trg_str_2dlist, original_idx_list, img, attr,type = batch
            ...
            beam_search_result = generator.beam_search(src, src_lens, src_oov, src_mask, oov_lists, img, attr,type)
...
def predict_seq2seq(opt, model=None):
    ...
    #opt.test_src_fn = '../data/{}/test_ocr_kw_prompt.txt'.format(opt.data_tag)
    #opt.test_src_fn = '../data/{}/test_ocr_kw.txt'.format(opt.data_tag)
    opt.test_src_fn = opt.test_src.format(opt.data_tag)
...
def run_classifier(model, data_loader, opt):
    ...
    with open(opt.pred_path, 'w') as fw:
        for batch_i, batch in enumerate(data_loader):
            ...
            output = model(src, src_lens, src_mask, src_oov, trg, trg_lens, trg_mask, max_num_oov, img, attr,type, only_classifier=True)
            # output = model(src, src_lens, src_mask, src_oov, trg, max_num_oov, img, attr,type, only_classifier=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data_tag', type=str, default='tw_mm_s1_ocr')
    parser.add_argument('-raw_data_path', type=str, default='../data/{}')
    parser.add_argument('-data_path', type=str, default='/home/dyfff/new/CMKP/data/{}')
    ...
    parser.add_argument('-copy_attn', type=bool, default=True, help='')
    parser.add_argument('-cur_model', type=str, default='mixture_text_img_multi_head_att_h4_d128')
