...
from multi_head_att.submodules import MultiHeadAttention, PositionwiseFeedForward, PositionalEncoding, MultiHeadAttention4Filtration
...
class MyMultiHeadAttention4Filtration(nn.Module):
    def __init__(self, d_model, d_kv, dropout=0.1, need_mask=False):
        super(MyMultiHeadAttention4Filtration, self).__init__()
        self.need_mask = need_mask
        self.slf_attn = MultiHeadAttention4Filtration( d_model, d_kv, d_kv, dropout=dropout)
        self.pos_ffn = PositionwiseFeedForward(d_model, d_model, dropout=dropout)

    def forward(self, q, k, v,gate=None,mask=None):
        ...
        enc_output, enc_slf_attn = self.slf_attn(q, k, v, gate)
        enc_output = self.pos_ffn(enc_output)
        #print(f'q enc_output:{enc_output.size()}')
        #print(f'v:{v.size()}')
        assert enc_output.size() == v.size()
